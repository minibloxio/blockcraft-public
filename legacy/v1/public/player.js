...
class Player {
	constructor(camera, blocks) {
		...
		// Body
		this.body = new THREE.Mesh(new THREE.BoxGeometry(blockSize*0.8, blockSize*1.8, blockSize*0.8), new THREE.MeshBasicMaterial({
		  color: 0xf1c27d,
		  castShadow: true
		}));
		...
		this.armExtend = false;
		this.arm = new THREE.Mesh(new THREE.BoxGeometry(1, 1, 5), new THREE.MeshPhongMaterial({
		  color: 0xf1c27d
		}));
	...
	mine() {
		...
		if (this.closest.object && this.click) {
			for (var i = 0; i < blocks.length; i++) {
				if (blocks[i].uuid === this.closest.object.uuid) {
					// Remove the block from the chunk
					var x = blocks[i].position.x
					var y = blocks[i].position.y
					var z = blocks[i].position.z
					chunks[0].matrix[x/blockSize][z/blockSize][y/blockSize] = "air";
		...
		for (var i = 0; i < axes.length; i++) {
			...
			if (axis === 'y' && !this.fly) {
				...
				for (var j = 0; j < tests; j++) {
					// Test for y
					this.controls.getObject().position['y'] += newMove['y']/tests;
					if (this.collisionHelper.collides() && this.velocity.y < 0) {
						this.onObject = true;
						newMove['y'] = 0;

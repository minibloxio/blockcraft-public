	...
	constructor (x, z) {
		...
		for(var i=this.world.x*blockSize;i<this.size+this.world.x*blockSize;i++) {
			for(var j=this.world.z*blockSize;j<this.size+this.world.z*blockSize;j++) {
				var ex = 0.5;
				this.matrix[i-this.world.x*blockSize][j-this.world.z*blockSize].height = Math.floor((((noise.simplex2(i/100,j/100)+(noise.simplex2((i+200)/50,j/50)*Math.pow(ex,1))+(noise.simplex2((i+400)/25,j/25)*Math.pow(ex,2))+
		...
		for (var x = 0; x < this.matrix.length; x++) {
			for (var z = 0; z < this.matrix[x].length; z++) {
				for (var y = 0; y < this.matrix[x][z].length; y++) {
					var isAir = false;
					if (y == 0) {
						this.matrix[x][z][y] = new Block(bedrock)
					} else if (this.matrix[x][z].height === y) {
						this.matrix[x][z][y] = new Block(grass)		
					} else if (y < this.matrix[x][z].height && y >= this.matrix[x][z].height-3) {
						this.matrix[x][z][y] = new Block(dirt)
					} else if (y < this.matrix[x][z].height-3) {
						this.matrix[x][z][y] = new Block(stone)
					} else {
...
*/
			...
			if (player) {
				//console.log((Date.now() - this.tick) + " " + blocks.length)
			}

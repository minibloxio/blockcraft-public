...
class Player {
	constructor(camera, blocks) {
		...
		// Inventory
		this.currentSlot = 0;
		this.toolbar = [2, 3, 4, 5, 6, 7, 0, 0, 14];
	}
    ...
    select(update) {
		...
		for (var i = 0; i < intersects.length; i++) {
			if (intersects[i].object.name != "wireframe")
	...
	placeBlock() {
		...
		if (this.closest.point && this.place) {
			...
			this.placeType = this.toolbar[this.currentSlot]
			if (this.placeType > 0) { // Place a block, not air
	...
	updateChunks() {
		...
		if (Date.now()-this.chunkTick > 0) {
			...
			for (var i = 0; i < this.chunkLoadingRate; i++) {
				...
				if (chunk) {
					updateCellGeometry(chunk.x*cellSize, chunk.y*cellSize, chunk.z*cellSize)

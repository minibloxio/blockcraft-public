    ...
    if (!this.cells[`${cellX},${cellY},${cellZ}`]) { // Check if chunk already exists
      ...
      for (let z = 0; z < cellSize; ++z) {
        for (let x = 0; x < cellSize; ++x) {
          ...
          const height = Math.floor((noise.noise2D(xPos/30, zPos/30)+1)*5)+30;
          // Add fauna
          let tree = Math.random() < 0.01

          // Add trees?
          if (tree) {
            for (let t of treeStruct) {
              this.setVoxel(xPos+t[0], height+t[1], zPos+t[2], t[3], false);
            }
            ...
            for (let y = 3; y <= 6; y++) {
              if (y == 3 || y == 4) {
                for (let x = -2; x <= 2; x++) {
                  for (let z = -2; z <= 2; z++) {
                    if (!(x == 0 && z == 0))
                      this.setVoxel(xPos+x, height+y, zPos+z, 7, false);
                  }
                }
              } else if (y == 5) {
                for (let x = -1; x <= 1; x++) {
                ...
                for (let x = -1; x <= 1; x++) {
                  this.setVoxel(xPos+x, height+y, zPos, 7, false);
                }
                for (let z = -1; z <= 1; z++) {
                  this.setVoxel(xPos, height+y, zPos+z, 7, false);
                }
              }
            }
          }

...
class Player {
	constructor(camera, blocks) {
		...
		this.click = false;
		this.place = false;
		this.placeType = 1; // Type of block being placed
	...
	placeBlock() {
		...
		if (this.closest.point && this.place) {
		    ...
		    if (this.collides() && !this.key.sneak) {
		    	world.setVoxel(x, y, z, 0);
		    	updateVoxelGeometry(x, y, z);
		    }
	...
	updateChunks() {
	    ...
	    for ( let i = 0; i < this.renderDistance*this.renderDistance; i++ ) {
	    	for (let y = 0; y < 3; y++) {
				...
				if (!world.cells[`${cellX},${cellY},${cellZ}`]) { // Check if chunk already exists
					this.chunksToLoad.push({
						x: cellX,
						y: cellY,
						z: cellZ
					})

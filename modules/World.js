...
module.exports = class World {
  ...
  init(options) {
    ...
    let disabledBlocksOffset = 2; // Number of blocks that can't be placed in the game
    this.blockOrderLegit = this.blockOrder.slice(disabledBlocksOffset, this.blockOrder.length);
  ...
  checkCollision(entity, players, io) {
    ...
    if (throwables.includes(entity.name)) {
      ...
      // Check if there is a voxel below the entity
      if (deltaVoxel) {

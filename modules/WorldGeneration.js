  ...
  generateCell(cellX, cellY, cellZ, world, exists) {
    ...
    for (let z = 0; z < cellSize; ++z) {
      for (let x = 0; x < cellSize; ++x) {
        ...
        for (let y = 0; y < cellSize; ++y) {
          ...
          if (world.getVoxel(xPos, yPos, zPos) > 0)
    ...
    for (let z = -3; z < cellSize + 3; ++z) {
      for (let x = -3; x < cellSize + 3; ++x) {
        ...
        // Add fauna
        let tree =
          rng1.noise3D(xPos / 30, height, zPos / 30) * rng1.noise2D(xPos, zPos) > moisture &&
          height > this.waterLevel &&
          height < this.mountainLevel;

        let ungrowable = ["OCEAN", "BEACH", "SCORCHED", "BARE", "SNOW"];
        if (ungrowable.indexOf(biome) > -1) tree = false;

        if ((rng1.noise3D(xPos * 0.05, height * caveSparsity, zPos * 0.05) + 1) / 2 <= 0.1) continue;
          ...
          for (let y = 3; y <= 6; y++) {
            if (y == 3 || y == 4) {
              for (let x = -2; x <= 2; x++) {
                for (let z = -2; z <= 2; z++) {
                  if (Math.floor((height + y) / cellSize) != cellY) continue;
                  if (!(x == 0 && z == 0)) world.setVoxel(xPos + x, height + y, zPos + z, world.blockId["leaves_oak"]);
                }
              }
            } else if (y == 5) {
              for (let x = -1; x <= 1; x++) {

...
export class TextureManager {
  ...
  setTexture(order) {
    ...
    this.block_atlas = texture.image;
  ...
  drawImageNet(ctx_, order, entities) {
    ...
    for (let entity of order) {
      ...
      if (b instanceof Array) {
        ...
        // Single block face with custom name
        this.drawImage(ctx_, this.blockFaces[b].image, index);
      } else if (Object.keys(this.colormap).includes(entity)) {
        // Custom color
  ...
  mergeItemTextures(order) {
    ...
    switch (textureType) {
      case "basic":
        this.materialItem = new THREE.MeshBasicMaterial(settings);
        break;
      case "lambert":
        this.materialItem = new THREE.MeshLambertMaterial(settings);
        break;
      case "phong":
        this.materialItem = new THREE.MeshPhongMaterial(settings);
        break;
      case "standard":
        this.materialItem = new THREE.MeshStandardMaterial(settings);
        break;
      case "toon":
        this.materialItem = new THREE.MeshToonMaterial(settings);
        break;
    }

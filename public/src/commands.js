...
function updateHints() {
  ...
  for (let name of world.blockOrderLegit) {
    commands.setblock.hints[name] = "Set block to " + name; // Update setblock hint
    commands.give.hints[name] = "Give 1 " + name + " to yourself"; // Update give hint
  }
  for (let name of world.itemOrder) {
    commands.give.hints[name] = "Give 1 " + name + " to yourself"; // Update give hint
  }
...
export function giveCommandHint(msg, autocomplete) {
  ...
  for (let i = 0; i < commandIds.length; i++) {
    ...
    if (command.startsWith(msg[0]) && command != msg[0] && msg.length == 1) {
      ...
      if (firstArgUnique) {
        // If the first argument is unique
...
function giveItem(msg) {
  ...
  let entity = world.blockIdLegit[item] || world.itemId[item];
